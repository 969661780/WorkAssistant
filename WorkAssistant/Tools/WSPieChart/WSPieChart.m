...
-(void)drawRect:(CGRect)rect{
    ...
    if (_descArr.count==_valueArr.count&&_descArr.count>0) {
        ...
        for (NSInteger i = 0; i<_descArr.count; i++) {
            ...
            [self drawText:[NSString stringWithFormat:@"%@:%ld proportion:%.1f%c",_descArr[i],[_valueArr[i] integerValue],present,'%'] andContext:contex atPoint:P_M(30+self.frame.size.width/2*(i%2), 20*(i/2  )+25+_chartArcLength*2) WithColor:[UIColor blackColor] andTextFontSize:8];
        }
